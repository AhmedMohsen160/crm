...
/**
 * سنة ٢٠٢٤ ناقصة عمدًا (§١٤).
 *
 * > «عشرة أشهر مفقودة من ٢٠٢٤ (٩٠ صفًا للسنة كلها مقابل ٩٦١ في ٢٠٢٥).
 * >  **علّم ٢٠٢٤ صراحةً بأنها ناقصة وامنع استخدامها كخط أساس.**»
 */
export const INCOMPLETE_YEARS = [2024];

export function isIncompleteYear(year: number): boolean {
  return INCOMPLETE_YEARS.includes(year);
}
